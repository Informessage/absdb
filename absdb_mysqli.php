...
class absdb{
	...
	function __construct($host, $user, $pass, $db){
		...
		if(mysqli_connect_error()) die('ABSDB Connect error ('.$this->connector->connect_errno().') '.$this->connector->connect_error());
	}
	...
	function preparse_prepared(){
		...
		for($idx=0;$idx<count($this->params);$idx++){
			# search for ? in query	
			if(!$pos = strPos($this->sQuery, '?', $pos+1)) break;
			# isolate value at index
			$param = $this->params[$idx];
			# test if value should actually be null, and set matches to null
			if(isSet($this->types[$idx-$offset]) && ((($this->types[$idx-$offset]=='i' || $this->types[$idx-$offset]=='d') && $param==='') || ($this->types[$idx-$offset]=='s' && $param=='--null'))) $param=null;
			# check null
	...
	function query($query=null, $types=null, $params=array(), $empty_result=''){
		...
		if(isSet($this->STMT) && $this->STMT){ // check for valid statement object
			$this->STMT->execute();
			if($this->connector->errno) $this->error();
